...
| `@Input()`<br>`rowClassFormatter: MTExRowClassFormatter` | Function to apply custom CSS classes to rows based on row data. |
...
#### `MTExRowClassFormatter`

Row class formatter to apply custom CSS classes to rows.

```ts
interface MTExRowClassFormatter {
...
```ts
type MTExColumnType = 'selection' | 'string' | 'boolean' | 'number' | 'date' | 'datepicker' | 'textarea' | 'tag' | 'button' | 'link' | 'image' | 'currency' | 'percent';
```
